...
#   CRYPTO_SYMBOL : símbolo da coluna no CSV  (ex: "BTC", "ETH", "SOL")
#   CRYPTO_LABEL  : nome por extenso para títulos (ex: "Bitcoin", "Ethereum")
#   ETF_DATE      : data de corte do ETF no formato 'AAAA-MM-DD'
# =============================================================================
CRYPTO_SYMBOL = "BTC"
CRYPTO_LABEL  = "Bitcoin"
ETF_DATE      = "2024-01-11"   # ETF spot aprovado em 10/01/2024, início efetivo 11/01/2024

# =============================================================================
# 1. CONFIGURAÇÃO E CARREGAMENTO DE DADOS
# =============================================================================
file_path = f'rets_{CRYPTO_SYMBOL}_VIX.csv'
...
# Filtrar apenas o ativo selecionado e garantir numérico
df = df[[CRYPTO_SYMBOL]].apply(pd.to_numeric, errors='coerce').dropna()

# =============================================================================
# 2. DEFINIÇÃO DOS PERÍODOS (Seguindo a Dissertação 0.4)
# =============================================================================
periods = {
    "Amostra Total (Full Sample)": df[CRYPTO_SYMBOL],
    f"Pré-ETF (até {ETF_DATE[:7]})": df.loc[df.index <= ETF_DATE, CRYPTO_SYMBOL],
    f"Pós-ETF (após {ETF_DATE[:7]})": df.loc[df.index > ETF_DATE, CRYPTO_SYMBOL]
}

# =============================================================================
# 3. CÁLCULO DE LIMITES GLOBAIS (Para garantir "Mesma Escala")
# =============================================================================
global_min_x = df[CRYPTO_SYMBOL].min()
global_max_x = df[CRYPTO_SYMBOL].max()
...
axes[2].set_xlabel(f"Retornos Logarítmicos de {CRYPTO_LABEL} (%)", fontsize=11)
plt.xlim(xlims) # Força a mesma escala visual horizontal

# Título Geral (Opcional, pode ser removido para documento ABNT)
# plt.suptitle(f"Distribuição Incondicional dos Retornos de {CRYPTO_LABEL}", fontsize=14)

plt.tight_layout()

# Salvar
output_filename = f"Distribuicao_Retornos_{CRYPTO_SYMBOL}_Periodos.png"
